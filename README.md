...
一、项目概述
本项目基于 Spring AI 框架构建，实现了一个具备多轮对话记忆、RAG（检索增强生成）、Function Calling（工具调用） 能力的 AI 智能体助手。核心功能包括：
带会话级记忆的多轮对话
...
配置 JDK 26 环境
替换 application.yaml 中的 OpenAI 兼容接口信息（api-key/base-url/model）
2. 项目启动
# 编译项目
mvn clean compile
...
<img width="840" height="319" alt="360截图20260527141430" src="https://github.com/user-attachments/assets/16dba0bc-ae59-45ec-afaa-1e89a7d03032" />
...
会话记忆：基于内存 ConcurrentHashMap 存储，重启后丢失（生产需替换为 Redis）
模拟向量：MockEmbeddingModel 仅用于测试，生产需替换为真实 Embedding 服务
跨域配置：控制器添加 @CrossOrigin(origins = "*")，生产需限制允许的域名
八、扩展方向
替换真实 Embedding 服务（如 OpenAI Embedding）
会话记忆持久化到 Redis / 数据库
优化 RAG 检索算法（使用 Redis 向量检索插件）
增加更多工具调用能力（如数据库查询、接口调用）
前端页面美化与功能扩展（如历史记录展示、多模型切换）
